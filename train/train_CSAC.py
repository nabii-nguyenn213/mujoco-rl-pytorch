...
from agents.CSAC import CSAC_Agent
from components.buffer import ReplayBuffer
from envs.env import make_env


class CSAC(TrainAgent):
    def __init__(self, config, rank=None):
        ...
        assert len(self.env.observation_space.shape) == 1, (
            "CSAC currently expects vector observations."
        )
        assert len(self.env.action_space.shape) == 1, (
            "CSAC currently expects continuous 1D Box action space."
        )

        self.obs_dim = self.env.observation_space.shape[0]
        self.act_dim = self.env.action_space.shape[0]

        self.agent = CSAC_Agent(self.config)
    ...
    def run(self):
        ...
        self.logger.info(f"Initialized CSAC on {self.env_id}")
        ...
        try:
            while self.global_step < self.total_timesteps:
                ...
                if self.global_step > 0 and self.global_step % self.save_every == 0:
                    ckpt_path = os.path.join(self.ckpt_dir, f"csac_step_{self.global_step}.pt")
                ...
                if self.global_step > 0 and self.global_step % self.eval_every == 0:
                    ...
                    if is_best:
                        best_path = os.path.join(self.best_dir, "csac_best.pt")
            ...
            final_path = os.path.join(self.model_dir, "csac_final.pt")
    ...
    def _save(self, path):
        ckpt = {
            ...
            "prev_actor": self.agent.prev_actor.state_dict(),
            "actor_optim": self.agent.actor_optimizer.state_dict(),
            "critic1_optim": self.agent.critic1_optimizer.state_dict(),
            "critic2_optim": self.agent.critic2_optimizer.state_dict(),
        }
